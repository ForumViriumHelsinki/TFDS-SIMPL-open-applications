...
describe('ToolbarWrapper.vue', () => {
  beforeEach(() => {
    ...
    // Default mock for fetch to prevent errors in tests that don't explicitly mock it
    (global.fetch as any).mockResolvedValue({
      ok: true,
      json: async () => ({ firstName: 'User', lastName: 'Name' }),
    });
  });
  ...
  it('renders SToolbar with correct id attribute', () => {
    const wrapper = mount(ToolbarWrapper, {
      props: {
        searchValue: '',
        currentUrl: '',
    ...
    expect(toolbar.attributes('id')).toBe('toolbar');
  });
  ...
  it('passes correct props to SToolbar', () => {
    ...
    expect(toolbar.props('showSearch')).toBe(true);
  ...
  it('passes searchValue prop to component', () => {
    const wrapper = mount(ToolbarWrapper, {
      props: {
    ...
    expect(wrapper.props('searchValue')).toBe('initial search');
  });
  ...
  it('uses default props when not provided', () => {
    ...
    expect(wrapper.props('searchValue')).toBe('');
