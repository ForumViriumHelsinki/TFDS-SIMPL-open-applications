...
import { getToken } from '@/services/localApi';
...
export const fetchTokenClientSide = async (keycloakToken: string | undefined) => {
  if (!keycloakToken) {
    try {
      keycloakToken = (await getToken())?.token;
    } catch (err) {
      return undefined;
    }
  }
...
export const handleOAuthCallback = async (ctx: APIContext): Promise<Response | null> => {
...
export const handleRefreshToken = async (ctx: APIContext): Promise<Response | null> => {
  const pkceCodeVerifier = setNewPkceCodeVerifier(ctx);
