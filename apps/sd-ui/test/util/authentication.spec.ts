...
describe('authentication utils', () => {
  ...
  it('deleteAuthenticationCookies should delete all auth cookies', () => {
    ...
    expect(cookies.delete).toHaveBeenCalledWith(ACCESS_TOKEN_COOKIE, { path: '/' });
    expect(cookies.delete).toHaveBeenCalledWith(REFRESH_TOKEN_COOKIE, { path: '/' });
    expect(cookies.delete).toHaveBeenCalledWith(ID_TOKEN_COOKIE, { path: '/' });
  });
  ...
  it('setCacheHeaders should set cache headers', () => {
    ...
    expect(jsonResponse.headers.get('X-Frame-Options')).toBe(null);
    expect(jsonResponse.headers.get('Content-Security-Policy')).toBe(null);
  });
