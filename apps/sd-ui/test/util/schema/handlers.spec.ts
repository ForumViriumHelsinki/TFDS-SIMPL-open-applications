...
describe('handlers.ts', () => {
  ...
  describe('handleNameProperty', () => {
    it('sets the title property for a field', () => {
      const quads: Quad[] = [];
      const node: Quad = {
        subject: { value: 'shapeNode' },
      ...
      handleNameProperty(quads, node, shapeObject, 'testProperty');

      expect(shapeObject.properties?.testProperty?.title).toBe('Test property');
    });
  });
