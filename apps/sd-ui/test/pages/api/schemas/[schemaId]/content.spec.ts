...
import { GET } from '@/pages/api/schemas/[schemaId]/content';
...
const createMockContext = (schemaId: string, schemaUIType?: string | null, token = 'test-token') => {
  const url = new URL(`http://localhost/api/schemas/${schemaId}/content`);
...
describe('GET /api/schemas/[schemaId]/content', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('calls safeServiceCall with fetchSchemaData, schemaId, and token', async () => {
    const textMock = vi.fn().mockResolvedValue('turtle content');
    mockSafeServiceCall.mockResolvedValue({ ok: true, text: textMock });
    mockBuildSchemaResponse.mockResolvedValue(new Response('ok', { status: 200 }));

    await GET(createMockContext('service-offeringShape', 'sdCreation'));
  ...
  it('returns the error response immediately when safeServiceCall fails', async () => {
    ...
    const result = await GET(createMockContext('bad-schema'));
  ...
  it('calls buildSchemaResponse with the response text and schemaUIType', async () => {
    ...
    const result = await GET(createMockContext('my-schema', 'sdCreation'));
  ...
  it('passes undefined token when cookie is missing', async () => {
    ...
    await GET(createMockContext('my-schema', null, ''));
